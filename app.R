...
server <- function(input, output, session) {
  output$cluster <- render_gt({
    ...
    if (!"all" %in% input$pitches) {
      ...
      # subtitle saying what counts as weak contact
    # take the hand column out 
    # movement is from catchers perspective
    # change what shows up for pitch type names and cluster names
